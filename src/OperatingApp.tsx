...
import ProjectRouteBridge from "./ProjectRouteBridge";
  ...
  useEffect(() => {
    const capture = async (event: MouseEvent) => {
      ...
      if (button.classList.contains("vote")) {
        ...
        setVoteBusy(true);
        try { const connection = await phantom.connect(); const walletAddress = connection.publicKey.toString(); const nonce = await createVoteNonce(project.slug, walletAddress); const signed = await phantom.signMessage(new TextEncoder().encode(nonce.message), "utf8"); const result = await submitVote({ nonce: nonce.nonce, walletAddress, signature: bytesToBase64(signed.signature) }); await refreshProjects(); await refreshActivity(); window.alert(`Vote counted. ${project.name} now has ${result.votes} verified vote${result.votes === 1 ? "" : "s"} this week.`); }
