...
export function SubmitProjectPanel({ onClose }: { onClose: () => void }) {
  ...
  async function runAnalysis() {
    ...
    setAnalyzing(true); setMessage(""); setSuccess(false); setAnalysis(null); setDuplicate(false);
    try {
      ...
      setMessage(`Valid Solana token identified.${metadataNote} Bonding status, graduation, liquidity and Jupiter routing do not prevent submission. Final approval is handled by SolPitch admin review.`);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Token analysis failed.";
      if (errorMessage === "This project has already been submitted." || errorMessage === "That contract address has already been submitted.") {
        setMessage("");
        setDuplicate(true);
      } else {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!listingEligible(analysis)) {
      setSuccess(false);
      setMessage("Analyze and confirm a valid Solana token mint before submitting it for review.");
      return;
    }
