...
function identifiableToken(result: TokenAnalysis | null) {
  return Boolean(result && (result.found || (result.name?.trim() && result.symbol?.trim())));
}
...
export function SubmitProjectPanel({ onClose }: { onClose: () => void }) {
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      setAnalysis(result);
      if (!identifiableToken(result)) {
        setMessage("SolPitch could not identify a valid Solana token from this contract address. Double-check the CA before continuing.");
        return;
      }
      ...
      setMessage(`Valid Solana token identified.${metadataNote} Review the prefilled information, complete anything missing, and submit it for admin review.`);
    } catch (error) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!identifiableToken(analysis)) {
      setSuccess(false);
      setMessage("Analyze and confirm a valid Solana token before submitting it for review.");
      return;
    }
  ...
  return <div className="workflow-overlay"><section className="workflow-shell"><button className="workflow-close" onClick={onClose}>×</button><span className="eyebrow">SUGGEST A LISTING</span><h1>Suggest a Solana project</h1><p className="workflow-intro">Paste a Solana contract address. SolPitch will identify the token and prefill whatever public project information is available. Suggestions remain private until approved by a SolPitch administrator.</p>{message && <div className={success ? "workflow-message success" : "workflow-message error"}>{message}</div>}<form ref={formRef} className="workflow-form" onSubmit={submit}><div className="workflow-grid"><label className="wide">Contract address<div className="contract-analyzer"><input name="contractAddress" required minLength={32} placeholder="Paste Solana contract address" onChange={()=>{setAnalysis(null);setMessage("");setSuccess(false)}}/><button type="button" onClick={()=>void runAnalysis()} disabled={analyzing}>{analyzing ? "Analyzing…" : "Analyze token"}</button></div></label>{identifiableToken(analysis) && analysis && <div className="token-analysis wide">{analysis.logoUrl ? <img src={analysis.logoUrl} alt=""/> : <div className="analysis-logo">{analysis.symbol?.slice(0,2) || "?"}</div>}<div><strong>{analysis.name || "Token identified"} {analysis.symbol ? `$${analysis.symbol}` : ""}</strong><small>Valid Solana token · Pending admin review</small>{analysis.found && <span>Liquidity {money(analysis.liquidityUsd)} · Market cap {money(analysis.marketCap)} · 24h volume {money(analysis.volume24h)}</span>}{analysis.metadataSource && <span>Metadata: {analysis.metadataSource} · {analysis.descriptionFound ? "Description found" : "Description not found"}</span>}</div>{analysis.dexScreenerUrl && <a href={analysis.dexScreenerUrl} target="_blank" rel="noreferrer">View public market ↗</a>}</div>}<label>Project name<input name="name" required maxLength={80}/></label><label>Ticker<input name="symbol" required maxLength={16} placeholder="$TOKEN"/></label><label>Project category<select name="category" defaultValue="Memecoin">{categories.map(category=><option value={category} key={category}>{category}</option>)}</select></label><label className="wide">Short pitch<input name="pitch" required maxLength={300} placeholder="Prefilled when public metadata includes a description"/></label><label className="wide">Full description<textarea name="description" required rows={5} maxLength={4900} placeholder="Prefilled from public metadata when available"/></label><label>Website<input name="website" type="url"/></label><label>X profile<input name="xUrl" type="url"/></label><label>Telegram<input name="telegramUrl" type="url"/></label></div><div className="workflow-message">Anyone may suggest a public-information listing. Submitting a CA does not publish it and does not guarantee approval. Every suggestion is reviewed privately by SolPitch first.</div><label className="workflow-check"><input type="checkbox" required/> I confirm the information shown is accurate to the best of my knowledge.</label><button className="primary workflow-submit" disabled={busy || analyzing || !identifiableToken(analysis)} type="submit">{busy ? "Submitting…" : "Submit for admin review"}</button></form></section></div>;
}
