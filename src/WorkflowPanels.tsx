...
export function SubmitProjectPanel({ onClose }: { onClose: () => void }) {
  ...
  function setField(name: string, value?: string) {
    ...
    if (!target) return;
    target.value = value?.trim() ? value.trim() : "";
  }
  async function runAnalysis() {
    ...
    try {
      ...
      setField("website", result.website);
      setField("xUrl", result.xUrl);
      setField("telegramUrl", result.telegramUrl);
      setField("description", result.description);
      setField("pitch", result.pitch);
      setSuccess(true);
