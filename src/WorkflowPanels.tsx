...
function listingEligible(result: TokenAnalysis | null) {
  return Boolean(result && result.found && result.tradable && Number(result.liquidityUsd ?? 0) > 0 && result.name?.trim() && result.symbol?.trim());
}
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      const metadataNote = result.descriptionFound
        ...
        : " No public description was found. Short and full descriptions are optional and can be added later by an admin or verified owner.";
      setMessage(`Active, liquid Solana token identified.${metadataNote} Review the prefilled information and submit it for admin review.`);
    } catch (error) {
