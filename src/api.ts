...
function normalizeLogoUrl(value: unknown) {
  ...
  // Extract CID from common IPFS forms
  const ipfsMatch =
...
export type SpotlightResponse = { entries:SpotlightEntry[] };
...
export async function getSpotlight(){const result=await request<{projects:Record<string,unknown>[]}>('/api/spotlight');return{entries:result.projects.map(row=>({rank:Number(row.rank??0),periodVotes:Number(row.period_votes??0),project:mapProjectRow({...row,votes:row.period_votes})}))} satisfies SpotlightResponse;}
