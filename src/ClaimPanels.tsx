...
export function ClaimProjectPanel({ project, onClose }: { project: Project; onClose: () => void }) {
  ...
  async function claim(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccess(true);
      setMessage("Signed claim received. SolPitch will review the wallet and available public project records before awarding the Verified Owner badge.");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Ownership claim failed.");
    } finally { setBusy(false); }
  }

  return <div className="workflow-overlay"><section className="workflow-shell claim-workflow"><button className="workflow-close" onClick={onClose}>×</button><span className="eyebrow">PHANTOM OWNERSHIP CLAIM</span><h1>Claim {project.name}</h1><p className="workflow-intro">Connect a recognized project wallet and sign a one-time message. This is not a transaction, costs no SOL, and cannot move funds. Contact information and supporting evidence are optional.</p>{message && <div className={`workflow-message ${success ? "success" : "error"}`}>{message}</div>}{!success && <form className="workflow-form" onSubmit={claim}><div className="claim-steps"><div><b>1</b><span><strong>Connect Phantom</strong><small>Use the creator, deployer, treasury, or publicly recognized project wallet.</small></span></div><div><b>2</b><span><strong>Sign the SolPitch message</strong><small>The exact project, wallet, and one-time nonce are included.</small></span></div><div><b>3</b><span><strong>Admin verification</strong><small>SolPitch reviews the signed wallet and any available public evidence.</small></span></div></div><label>Contact email <small>(optional)</small><input name="email" type="email" placeholder="owner@project.com" /></label><label>Public ownership evidence URL <small>(optional)</small><input name="evidenceUrl" type="url" placeholder="Official X post, website, launch page, or explorer evidence" /></label><div className="workflow-message">After approval, the verified owner can manage optional project profile details. Contract address and core token identity remain protected by admin review.</div><button className="primary workflow-submit" disabled={busy} type="submit">{busy ? "Connecting and signing…" : "Connect Phantom and sign"}</button></form>}</section></div>;
}
