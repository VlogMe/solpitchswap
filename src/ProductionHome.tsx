...
type Filter = "all" | "swap" | "featured" | "newest" | "most-voted" | ProjectCategory;
...
function ProjectCard({
  ...
  return <article className="listing-card" data-project-slug={project.slug}>
    <div className="listing-top"><ProjectAvatar project={project}/><div className="coin-title"><div><h3>{project.name}</h3><span>${project.symbol}</span></div><div className="tag-row"><span className="status status-launched">{project.category}</span>{rank <= 10 && <span className="top-ten">Top 10</span>}{project.addedToSwap && <span className="claim-verified">Swap enabled</span>}{project.promoted && <span className="featured">Featured</span>}</div></div><button className={`favorite ${favorite ? "selected" : ""}`} onClick={onFavorite}>{favorite ? "♥" : "♡"}</button><button className="vote animated-count">▲ {project.votes}</button></div>
...
export default function ProductionHome({ projects, loading, error, onRetry }: { projects: Project[]; loading: boolean; error: string; onRetry: () => void }) {
  ...
  const filtered=useMemo(()=>{
    let result=projects;
    if(filter==="swap")result=result.filter(project=>project.addedToSwap);
