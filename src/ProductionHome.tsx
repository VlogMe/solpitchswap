...
function ProjectCard({ project, votes, favorite, topTen, onVote, onFavorite, onOpen, onClaim, onHover }: {
  project: Project; votes: number; favorite: boolean; topTen: boolean; onVote: () => void; onFavorite: () => void; onOpen: () => void; onClaim: () => void; onHover: () => void;
}) {
  const canTrade = tradable.has(project.projectStatus);
  return <article className="listing-card" onMouseEnter={onHover}>
...
function QuickView({ project, votes, onOpen, onVote }: { project: Project; votes: number; onOpen: () => void; onVote: () => void }) {
  const canTrade = tradable.has(project.projectStatus);
  return <section className="side-widget quick-view-card" key={project.slug}>
    <span className="eyebrow">QUICK VIEW</span>
    <div className="quick-view-head"><ProjectAvatar project={project} /><div><h3>{project.name}</h3><small>${project.symbol}</small></div></div>
    ...
    <p>{project.pitch}</p>
    <div className="quick-view-stats"><div><small>Market cap</small><strong>{project.marketCap}</strong></div><div><small>Liquidity</small><strong>{project.liquidity}</strong></div></div>
    <button className="quick-ca" onClick={() => navigator.clipboard?.writeText(project.contractAddress)}>{project.contractAddress.slice(0,8)}…{project.contractAddress.slice(-6)} <b>Copy</b></button>
    <div className="quick-actions"><button onClick={onVote}>▲ {votes}</button><button onClick={onOpen}>View</button>{canTrade && <a href={SWAP_URL}>Swap</a>}</div>
  </section>;
}
  ...
  return <div className="production-app">
    ...
    <div className="production-grid">
      ...
      {selected ? <ProjectPage project={selected} votes={votes[selected.slug]??0} onBack={()=>setSelected(null)} onVote={()=>vote(selected)} onClaim={()=>setClaiming(selected)} /> : <main className="project-feed">
        ...
        <section className="trending-strip"><span>🔥 Trending</span>{ranked.slice(0,5).map((project,index)=><button key={project.slug} onClick={()=>setSelected(project)} onMouseEnter={()=>setHovered(project)}><b>${project.symbol}</b><em>{index===0?"#1 THIS WEEK":`▲ ${index+3}%`}</em></button>)}</section>
        ...
        {filtered.map(project=><ProjectCard key={project.slug} project={project} votes={votes[project.slug]??0} favorite={favorites.has(project.slug)} topTen={ranked.slice(0,10).some(p=>p.slug===project.slug)} onVote={()=>vote(project)} onFavorite={()=>favorite(project)} onOpen={()=>setSelected(project)} onClaim={()=>setClaiming(project)} onHover={()=>setHovered(project)} />)}
      </main>}
      <aside className="utility-rail">
        ...
        <QuickView project={hovered} votes={votes[hovered.slug]??0} onOpen={()=>setSelected(hovered)} onVote={()=>vote(hovered)} />
