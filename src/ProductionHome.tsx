...
function ProjectCard({
  ...
  return <article className="listing-card" data-project-slug={project.slug}>
    ...
    <div className="listing-footer"><div className="listing-actions"><button className="ghost" onClick={onOpen}>View project</button>{project.claimStatus === "unclaimed" && <button className="claim-button">Claim project</button>}<a href={`${SWAP_URL}?outputMint=${encodeURIComponent(project.contractAddress)}`} target="solpitch-swap-frame">Buy ${project.symbol}</a></div></div>
  </article>;
}
