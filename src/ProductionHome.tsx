...
function ProjectCard({
  ...
  return (
    <article className="listing-card">
      ...
      <p>{project.pitch}</p>
      <button className="ca" onClick={() => navigator.clipboard?.writeText(project.contractAddress)}>
        {project.contractAddress.slice(0, 10)}…{project.contractAddress.slice(-7)} <b>Copy CA</b>
      </button>
      <div className="stats">
      ...
      <div className="listing-footer">
        <div className="trust-note">{project.claimStatus === "verified" ? "Information maintained by the verified project owner." : "Public community listing. Not yet controlled by the project team."}</div>
    ...
    <main className="project-detail">
      ...
      <section className="detail-hero">
        ...
        <div className="detail-identity">
          <div className="detail-avatar">{project.symbol.slice(0, 2)}</div>
          <div>
        ...
        <div className="detail-actions">
          ...
          {project.claimStatus === "unclaimed" && <button className="claim-button" onClick={onClaim}>Claim this project</button>}
      ...
      <div className="detail-columns">
        ...
        <section className="detail-panel claim-panel"><span className="eyebrow">OWNERSHIP</span><h2>{project.claimStatus === "verified" ? "Official page" : "This page is claimable"}</h2><p>{project.claimStatus === "verified" ? "The project owner has been verified. Owner-supplied information remains separate from SolPitch moderation and risk notices." : "The legitimate project owner can claim this page by connecting a project wallet, signing a free message, and completing review."}</p>{project.claimStatus === "unclaimed" && <button className="primary wide" onClick={onClaim}>Start free claim</button>}</section>
      </div>
...
function ClaimDialog({ project, onClose }: { project: Project; onClose: () => void }) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className="claim-modal" role="dialog" aria-modal="true" onMouseDown={event => event.stopPropagation()}><button className="modal-close" onClick={onClose}>×</button><span className="eyebrow">CLAIM PROJECT</span><h2>Verify ownership of {project.name}</h2><p>Connect the recognized project wallet and sign a one-time message. Signing is free and cannot move funds.</p><div className="claim-steps"><div><b>1</b><span><strong>Connect wallet</strong><small>Use Phantom or another supported Solana wallet.</small></span></div><div><b>2</b><span><strong>Sign a message</strong><small>No transaction, gas fee, or token approval.</small></span></div><div><b>3</b><span><strong>Ownership review</strong><small>We compare the wallet with public creator and authority records.</small></span></div></div><button className="primary wide" disabled>Wallet verification coming next</button><small className="security-copy">Never share a seed phrase or private key. SolPitch will never ask you to send SOL to claim a page.</small></section></div>;
}
...
export default function ProductionHome() {
  ...
  return <div className="production-app">
    ...
    <div className="production-grid">
      ...
      <aside className="utility-rail"><section className="swap-widget"><div className="widget-title"><div><span className="eyebrow">SOLPITCH</span><h2>Swap</h2></div><span className="live-pill">LIVE</span></div><div className="swap-field"><small>You pay</small><div><strong>0.00</strong><button>◎ SOL</button></div></div><div className="switch">↓</div><div className="swap-field"><small>You receive</small><div><strong>0.00</strong><button>Select token</button></div></div><a href={SWAP_URL}>Open SolPitch Swap</a><p>Uses the existing protected swap at solpitch.net.</p></section><section className="side-widget"><div className="widget-title"><h3>Promoted</h3><button>Advertise</button></div><div className="side-empty"><b>Paid placements will appear here</b><span>Promotion never means endorsement.</span></div></section><section className="side-widget"><div className="widget-title"><h3>Most voted</h3><span>Community</span></div>{projects.slice().sort((a, b) => (votes[b.slug] ?? b.votes) - (votes[a.slug] ?? a.votes)).slice(0, 5).map((project, index) => <button className="rank" onClick={() => setSelected(project)} key={project.slug}><b>{index + 1}</b><div className="mini-avatar">{project.symbol.slice(0, 1)}</div><span><strong>{project.name}</strong><small>${project.symbol}</small></span><em>▲ {votes[project.slug] ?? project.votes}</em></button>)}</section></aside>
    </div>
