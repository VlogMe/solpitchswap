...
function validateSubmission(input: SubmissionInput) {
  ...
  for (const value of [input.website, input.xUrl, input.telegramUrl, input.statusProofUrl]) {
    if (!value) continue; try { new URL(value); } catch { return "One or more URLs are invalid."; }
  }
  return null;
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/api/admin/login" && request.method === "POST") {
      ...
      return json({ ok: true }, 200, { ...cors, "set-cookie": `${SESSION_COOKIE}=${token}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=${SESSION_TTL_SECONDS}` });
    }
    ...
    if (url.pathname === "/api/admin/submissions" && request.method === "GET") {
      ...
      const statement = projectStatus && PROJECT_STATUSES.includes(projectStatus as ProjectStatus)
        ? env.DB.prepare("SELECT * FROM submissions WHERE status = ?1 AND project_status = ?2 ORDER BY created_at DESC LIMIT 200").bind(status, projectStatus)
        : env.DB.prepare("SELECT * FROM submissions WHERE status = ?1 ORDER BY created_at DESC LIMIT 200").bind(status);
      const results = await statement.all(); return json({ submissions: results.results }, 200, cors);
    }
    ...
    if (reviewMatch && request.method === "PATCH") {
      ...
      await env.DB.prepare("UPDATE submissions SET status = ?1, reviewer_notes = ?2, reviewed_at = CURRENT_TIMESTAMP WHERE id = ?3").bind(body.status, body.reviewerNotes?.trim() || null, reviewMatch[1]).run();
